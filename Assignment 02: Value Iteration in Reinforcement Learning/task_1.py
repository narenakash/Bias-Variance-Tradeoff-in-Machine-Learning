nStates = 60
nActions = 3

arr = [0.5, 1, 2]
X = 29 #Team Number

states = []
actions = ['SHOOT', 'DODGE', 'RECHARGE']
transition_model = {}
gamma = 0.99
delta = 0.001
penalty = -10/arr[X % 3]
reward = {}

for i in range(0, 5):
    for j in range(0, 3):
        for k in range(0, 4):
            states.append((i, j, k))

for state in states:
    h, a, s = state
    reward[state] = penalty if h != 0 else 10

# for rkey, rval in reward.items():
#     print(rkey, rval)
# input()

# print(len(states))
# input()



for state in states:
    """For each of the states in the Markov Decision Process, we assign 
    the transition pobabilities to the next possible states according
    to the rules given in the question
    """
    transition = [[], [], []]
    h, a, s = state
    if a > 0:
        if s - 1 >= 0 and h - 1 >= 0:
            # transition[0].append((0.5, (a-1, s-1, h-1)))
            # transition[0].append((0.5, (a-1, s-1, h)))
            transition[0].append((0.5, (h-1, a-1, s-1)))
        if s - 1 >= 0:
            transition[0].append((0.5, (h, a-1, s-1)))
    if s == 2:
        if a + 1 <= 2:
            # transition[1].append((0.64, (a+1, s-1, h)))
            # transition[1].append((0.16, (a+1, s-2, h)))
            transition[1].append((0.64, (h, a+1, s-1)))
            transition[1].append((0.16, (h, a+1, s-2)))
        # transition[1].append((0.16, (a, s-1, h)))
        # transition[1].append((0.04, (a, s-2, h)))
        transition[1].append((0.16, (h, a, s-1)))
        transition[1].append((0.04, (h, a, s-2)))
    if s == 1:
        if a + 1 <= 2:
            # transition[1].append((0.8, (a+1, s-1, h)))
            transition[1].append((0.8, (h, a+1, s-1)))
        # transition[1].append((0.2, (a, s-1, h)))
        transition[1].append((0.2, (h, a, s-1)))
    if s + 1 <= 3:
        # transition[2].append((0.8, (a, s+1, h)))
        transition[2].append((0.8, (h, a, s+1)))
    # transition[2].append((0.2, (a, s, h)))
    transition[2].append((0.2, (h, a, s)))

    transition_model[state] = transition


def print_transition_model():
    """For each state of the Markov Decision Process environment, the
    transition model is of the form ACTION: (probability, state), <(probability, state), ...>
    """
    for key, val in transition_model.items():
        print("State: " + str(key))
        print("SHOOT: " + str(val[0]))
        print("DODGE: " + str(val[1]))
        print("RECHARGE: " + str(val[2]) + "\n")

# print_transition_model()
# input()

def value_iteration():
    count = 0
    U1 = dict([(s, 0) for s in states])
    R, T, G = reward, transition_model, gamma
    while True:
        print("iteration="+str(count))
        count = count + 1
        U = U1.copy()
        diff = 0
        for s in states:
            # U1[s] = R[s] + G * max([sum([p * U[s1] for (p, s1) in T(s, a)]) for a in actions])
            # print(s)
            # for a in range(0, 3):
            #     print(a)
            #     for p, s1 in T[s][a]:
            #         print(s1)
            # print("\n\n")
            U1[s] = R[s] + G * max([sum([p * U[s1] for p, s1 in T[s][a]]) for a in range(0, 3)])
            diff = max(diff, abs(U1[s] - U[s]))
        if diff < delta * (1 - G) / G:
            return U

value_iteration()


